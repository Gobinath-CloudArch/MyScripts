<#
.SYNOPSIS
    Installation Master Dashboard v8.52 (Enterprise Deep-Hunt Framework)
 #>
...
Write-Host "[INIT] Booting Enterprise Dashboard v8.52..." -ForegroundColor Cyan
...
function global:Check-Updates {
    global:Show-ExecutionLogs
    global:Write-Terminal "Checking GitHub repository for latest payload updates..." "Yellow" "ALL"
    global:Update-Status "Checking for updates..."
    
    try {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        $repoUrl = "https://api.github.com/repos/gobikrish90/MyScripts/contents/"
        $headers = @{ "User-Agent" = "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" }
        $contents = Invoke-RestMethod -Uri $repoUrl -Headers $headers -UseBasicParsing -ErrorAction Stop

        $latestPayload = $contents | Where-Object { $_.name -match "(?i)Phoenix Installation Master.*\.zip$" } | Sort-Object name -Descending | Select-Object -First 1

        if ($latestPayload) {
            $localPayloadVerFile = "C:\PnxTemp\CurrentPayloadVersion.txt"
            $currentPayloadVer = if (Test-Path $localPayloadVerFile) { (Get-Content $localPayloadVerFile).Trim() } else { "NONE" }

            if ($latestPayload.name -ne $currentPayloadVer) {
                global:Write-Terminal "Found new payload update on GitHub: $($latestPayload.name)" "Cyan" "ALL"
                $TempZip = "C:\PnxTemp\$($latestPayload.name)"
                Invoke-WebRequest -Uri $latestPayload.download_url -OutFile $TempZip -Headers $headers -UseBasicParsing
                ...
                if (Test-Path $TempZip) {
                    ...
                    Set-Content -Path $localPayloadVerFile -Value $latestPayload.name -Force
                    ...
                    $script:ToolsLoaded = $true
                    global:Refresh-List
                    global:Update-Status "Dashboard refreshed with latest update."
                    global:Write-Terminal "Dashboard refreshed with latest update." "Lime" "ALL"
                }
            } else {
                global:Write-Terminal "No new update available." "Lime" "ALL"
                global:Update-Status "No new update available."
            }
        } else {
            global:Write-Terminal "No new update available." "Yellow" "ALL"
            global:Update-Status "No new update available."
        }
    } catch {
        global:Write-Terminal "Update check failed or no connection: $($_.Exception.Message)" "Red" "ALL"
        global:Update-Status "No new update available."
    }
}
...
function global:Switch-NewServerDashboard {
    global:Show-ExecutionLogs
    global:Write-Terminal "Searching for New Server Dashboard package in C:\PnxTemp..." "Cyan" "ALL"
    
    $searchLocations = @("C:\PnxTemp", "C:\PnxTemp\Phoenix Installation Master", $script:CurrentToolPath)
    $zipFile = $null

    foreach ($loc in $searchLocations) {
        if (Test-Path $loc) {
            $found = Get-ChildItem -Path $loc -Filter "*.zip" -Recurse -ErrorAction SilentlyContinue | 
                     Where-Object { $_.Name -match "(?i)New\s*Server|Dashboard" } | 
    ...
    if (-not $zipFile) {
        global:Write-Terminal "Package zip not found automatically. Opening File Dialog..." "Yellow" "ALL"
        $dlg = New-Object System.Windows.Forms.OpenFileDialog
    ...
    if ($zipFile -and (Test-Path $zipFile)) {
        ...
        global:Write-Terminal "Extracting New Server Dashboard from '$zipFile'..." "Cyan" "ALL"
        try {
            Expand-Archive -Path $zipFile -DestinationPath $extractDir -Force
            global:Unblock-ExtractedFiles $extractDir $true
            
            # Find DB script matching exactly the reference to image_668324.png structure
            $dbScript = Get-ChildItem -Path $extractDir -Filter "*DB Script*.ps1" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
            
            if (-not $dbScript) {
                $dbScript = Get-ChildItem -Path $extractDir -Filter "*.ps1" -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -match "(?i)DB" } | Select-Object -First 1
            }

            if ($dbScript) {
                global:Write-Terminal "Found DB Script: '$($dbScript.Name)'. Launching as Administrator..." "Lime" "ALL"
                global:Update-Status "Launching DB Script..."
                Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$($dbScript.FullName)`"" -WorkingDirectory $dbScript.DirectoryName -Verb RunAs
            } else {
                global:Write-Terminal "ERROR: Could not locate '*DB Script*.ps1' inside the zip archive." "Red" "ALL"
                global:Update-Status "DB Script missing in archive." $true
            }
        } catch {
        ...
        global:Write-Terminal "New Server Dashboard operation cancelled." "Yellow" "ALL"
    }
}

function global:Show-LicenseVerification {
    $licForm = New-Object System.Windows.Forms.Form
    $licForm.Text = "License Configuration Diff Tool"
    $licForm.Size = New-Object System.Drawing.Size(1000, 800)
    ...
    $lblOld.Text = "OLD Configuration"
    $lblOld.Location = New-Object System.Drawing.Point(30, 20)
    $lblOld.AutoSize = $true
    $lblOld.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    $licForm.Controls.Add($lblOld)

    $lblNew = New-Object System.Windows.Forms.Label
    $lblNew.Text = "NEW Configuration"
    $lblNew.Location = New-Object System.Drawing.Point(500, 20)
    $lblNew.AutoSize = $true
    $lblNew.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    $licForm.Controls.Add($lblNew)

    $txtOld = New-Object System.Windows.Forms.TextBox
    $txtOld.Location = New-Object System.Drawing.Point(30, 45)
    $txtOld.Size = New-Object System.Drawing.Size(450, 280)
    $txtOld.Multiline = $true; $txtOld.ScrollBars = "Vertical"
    $txtOld.BackColor = [System.Drawing.Color]::FromArgb(255, 5, 10, 31)
    $txtOld.ForeColor = [System.Drawing.Color]::White
    $txtOld.Font = New-Object System.Drawing.Font("Consolas", 9)
    $licForm.Controls.Add($txtOld)

    $txtNew = New-Object System.Windows.Forms.TextBox
    $txtNew.Location = New-Object System.Drawing.Point(500, 45)
    $txtNew.Size = New-Object System.Drawing.Size(450, 280)
    $txtNew.Multiline = $true; $txtNew.ScrollBars = "Vertical"
    $txtNew.BackColor = [System.Drawing.Color]::FromArgb(255, 5, 10, 31)
    $txtNew.ForeColor = [System.Drawing.Color]::White
    $txtNew.Font = New-Object System.Drawing.Font("Consolas", 9)
    $licForm.Controls.Add($txtNew)

    $btnCompare = New-Object System.Windows.Forms.Button
    $btnCompare.Text = "COMPARE CONFIGURATIONS"
    $btnCompare.Location = New-Object System.Drawing.Point(30, 340)
    $btnCompare.Size = New-Object System.Drawing.Size(780, 40)
    $btnCompare.BackColor = [System.Drawing.Color]::FromArgb(255, 0, 229, 255)
    $btnCompare.ForeColor = [System.Drawing.Color]::FromArgb(255, 11, 19, 43)
    $btnCompare.FlatStyle = "Flat"
    $btnCompare.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
    $licForm.Controls.Add($btnCompare)

    $btnClear = New-Object System.Windows.Forms.Button
    $btnClear.Text = "CLEAR"
    $btnClear.Location = New-Object System.Drawing.Point(820, 340)
    $btnClear.Size = New-Object System.Drawing.Size(130, 40)
    $btnClear.BackColor = [System.Drawing.Color]::FromArgb(255, 28, 37, 65)
    $btnClear.ForeColor = [System.Drawing.Color]::White
    $btnClear.FlatStyle = "Flat"
    $btnClear.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
    $licForm.Controls.Add($btnClear)

    $lblRes = New-Object System.Windows.Forms.Label
    $lblRes.Text = "VERIFICATION RESULTS"
    $lblRes.Location = New-Object System.Drawing.Point(30, 395)
    $lblRes.AutoSize = $true
    $lblRes.ForeColor = [System.Drawing.Color]::FromArgb(255, 91, 192, 190)
    $lblRes.Font = New-Object System.Drawing.Font("Consolas", 12, [System.Drawing.FontStyle]::Bold)
    $licForm.Controls.Add($lblRes)

    $rtbResults = New-Object System.Windows.Forms.RichTextBox
    $rtbResults.Location = New-Object System.Drawing.Point(30, 420)
    $rtbResults.Size = New-Object System.Drawing.Size(920, 310)
    $rtbResults.BackColor = [System.Drawing.Color]::FromArgb(255, 5, 10, 31)
    $rtbResults.ForeColor = [System.Drawing.Color]::White
    $rtbResults.Font = New-Object System.Drawing.Font("Consolas", 10)
    $rtbResults.ReadOnly = $true
    $licForm.Controls.Add($rtbResults)

    $btnClear.Add_Click({
        $txtOld.Clear(); $txtNew.Clear(); $rtbResults.Clear()
    })
    ...
    $parseConfigBlock = {
        ...
        foreach ($line in $lines) {
            ...
            if ($trimmed -match "^Packages Name\s*=\s*(.+)$") {
                $currentPkg = $matches[1].Trim()
                $lastHeaderKey = $null
                if (-not $data.Packages.ContainsKey($currentPkg)) {
                    $data.Packages[$currentPkg] = $true
                    $data.PackageFeatures[$currentPkg] = @{}
                    $data.PackageJuris[$currentPkg] = New-Object System.Collections.Generic.HashSet[string]
                }
            }
            elseif ($trimmed -match "^Packages ID\s*=\s*(.+)$" -and $currentPkg) {
                $data.PackageIds[$currentPkg] = $matches[1].Trim()
            }
            elseif ($trimmed -match "^No\. of User\s*=\s*(.+)$" -and $currentPkg) {
                $data.PackageUsers[$currentPkg] = $matches[1].Trim()
            }
            elseif ($trimmed -match "^Package Juris\s*=\s*<([^>]*)>" -and $currentPkg) {
                $ids = $matches[1] -split ','
                foreach ($id in $ids) {
                    $tId = $id.Trim()
                    if ($tId) { [void]$data.PackageJuris[$currentPkg].Add($tId) }
                }
            }
            elseif ($trimmed -match "^(\d+)\s*-\s*(.+)$" -and $currentPkg) {
                $code = $matches[1].Trim()
                $desc = $matches[2].Trim()
            ...
            elseif ($lastHeaderKey -and -not $currentPkg) {
                $data.Header[$lastHeaderKey] += " " + $trimmed
            }
        }

        if ($data.Header.ContainsKey('Juris') -and $data.Header['Juris']) {
            $rawJuris = $data.Header['Juris']
            $entries = $rawJuris -split ','
            foreach ($entry in $entries) {
                $trimmedEntry = $entry.Trim()
                if ($trimmedEntry -match "\*") {
                    $parts = $trimmedEntry -split "\*", 2
                    $data.JurisMap[$parts[0].Trim()] = $parts[1].Trim()
                } elseif ($trimmedEntry) {
    ...
    $btnCompare.Add_Click({
        $rtbResults.Clear()
        $oldTextStr = $txtOld.Text; $newTextStr = $txtNew.Text
        
        if ([string]::IsNullOrWhiteSpace($oldTextStr) -or [string]::IsNullOrWhiteSpace($newTextStr)) {
            $rtbResults.SelectionColor = [System.Drawing.Color]::Yellow
            $rtbResults.AppendText("Please populate both OLD and NEW text fields to run a comparison.`n")
            return
        }

        $oldData = &$parseConfigBlock $oldTextStr
        $newData = &$parseConfigBlock $newTextStr
        $hasDiff = $false

        function Append-Text($text, $color, $bold = $false) {
            $rtbResults.SelectionStart = $rtbResults.TextLength
            $rtbResults.SelectionLength = 0
            $rtbResults.SelectionColor = $color
            if ($bold) { $rtbResults.SelectionFont = New-Object System.Drawing.Font("Consolas", 10, [System.Drawing.FontStyle]::Bold) } 
            else { $rtbResults.SelectionFont = New-Object System.Drawing.Font("Consolas", 10, [System.Drawing.FontStyle]::Regular) }
            $rtbResults.AppendText($text + "`n")
            $rtbResults.SelectionFont = New-Object System.Drawing.Font("Consolas", 10, [System.Drawing.FontStyle]::Regular)
        }

        # 1. Header Metadata Diff
        $headerKeys = @($oldData.Header.Keys) + @($newData.Header.Keys) | Select-Object -Unique | Where-Object { $_ -ne 'Juris' }
        $headerDiffs = @()
        foreach ($key in $headerKeys) {
            $oVal = if ($oldData.Header.ContainsKey($key)) { $oldData.Header[$key] } else { "(none)" }
            $nVal = if ($newData.Header.ContainsKey($key)) { $newData.Header[$key] } else { "(none)" }
            if ($oVal -ne $nVal) {
                $headerDiffs += [PSCustomObject]@{ Key = $key; OldVal = $oVal; NewVal = $nVal }
            }
        }

        if ($headerDiffs.Count -gt 0) {
            $hasDiff = $true
            Append-Text "=== HEADER METADATA CHANGES ===" [System.Drawing.Color]::Cyan $true
            foreach ($d in $headerDiffs) {
                Append-Text "  [CHANGED] $($d.Key): $($d.OldVal) -> $($d.NewVal)" [System.Drawing.Color]::Yellow
            }
            Append-Text "" [System.Drawing.Color]::White
        }

        # 2. Top-Level Jurisdiction Changes
        $jurisKeys = @($oldData.JurisMap.Keys) + @($newData.JurisMap.Keys) | Select-Object -Unique | Sort-Object
        $jurisDiffs = @()
        foreach ($id in $jurisKeys) {
            $oName = if ($oldData.JurisMap.ContainsKey($id)) { $oldData.JurisMap[$id] } else { $null }
            $nName = if ($newData.JurisMap.ContainsKey($id)) { $newData.JurisMap[$id] } else { $null }

            if ($null -eq $oName) {
                $jurisDiffs += [PSCustomObject]@{ Type="Added"; Text="[+ JURIS ADDED] ID $($id): $nName"; Color=[System.Drawing.Color]::Lime }
            } elseif ($null -eq $nName) {
                $jurisDiffs += [PSCustomObject]@{ Type="Removed"; Text="[- JURIS REMOVED] ID $($id): $oName"; Color=[System.Drawing.Color]::FromArgb(255, 76, 76) }
            } elseif ($oName -ne $nName) {
                $jurisDiffs += [PSCustomObject]@{ Type="Modified"; Text="[NAME CHANGED] ID $($id): $oName -> $nName"; Color=[System.Drawing.Color]::Yellow }
            }
        }

        if ($jurisDiffs.Count -gt 0) {
            $hasDiff = $true
            Append-Text "=== TOP-LEVEL JURISDICTION CHANGES ===" [System.Drawing.Color]::Cyan $true
            foreach ($jd in $jurisDiffs) { Append-Text "  $($jd.Text)" $jd.Color }
            Append-Text "" [System.Drawing.Color]::White
        }

        # 3. Package & Feature Differences
        $allPkgs = @($oldData.Packages.Keys) + @($newData.Packages.Keys) | Select-Object -Unique | Sort-Object
        
        foreach ($pkg in $allPkgs) {
            if (-not $oldData.Packages.ContainsKey($pkg)) {
                $hasDiff = $true
                Append-Text "=== PACKAGE ADDED: $pkg ===" [System.Drawing.Color]::Lime $true
                if ($newData.PackageUsers.ContainsKey($pkg)) {
                    Append-Text "  [+] No. of User = $($newData.PackageUsers[$pkg])" [System.Drawing.Color]::Lime
                }
                foreach ($code in $newData.PackageFeatures[$pkg].Keys | Sort-Object) {
                    Append-Text "  [+] $code - $($newData.PackageFeatures[$pkg][$code])" [System.Drawing.Color]::Lime
                }
                Append-Text "" [System.Drawing.Color]::White
            }
            elseif (-not $newData.Packages.ContainsKey($pkg)) {
                $hasDiff = $true
                Append-Text "=== PACKAGE REMOVED: $pkg ===" [System.Drawing.Color]::FromArgb(255, 76, 76) $true
                foreach ($code in $oldData.PackageFeatures[$pkg].Keys | Sort-Object) {
                    Append-Text "  [-] $code - $($oldData.PackageFeatures[$pkg][$code])" [System.Drawing.Color]::FromArgb(255, 76, 76)
                }
                Append-Text "" [System.Drawing.Color]::White
            }
            else {
                $oldFeats = $oldData.PackageFeatures[$pkg]
                $newFeats = $newData.PackageFeatures[$pkg]
                $allCodes = @($oldFeats.Keys) + @($newFeats.Keys) | Select-Object -Unique | Sort-Object

                $featureDiffs = @()
                foreach ($code in $allCodes) {
                    $oDesc = if ($oldFeats.ContainsKey($code)) { $oldFeats[$code] } else { $null }
                    $nDesc = if ($newFeats.ContainsKey($code)) { $newFeats[$code] } else { $null }

                    if ($null -eq $oDesc) {
                        $featureDiffs += [PSCustomObject]@{ Text="[+ ADDED] $code - $nDesc"; Color=[System.Drawing.Color]::Lime }
                    } elseif ($null -eq $nDesc) {
                        $featureDiffs += [PSCustomObject]@{ Text="[- REMOVED] $code - $oDesc"; Color=[System.Drawing.Color]::FromArgb(255, 76, 76) }
                    } elseif ($oDesc -ne $nDesc) {
                        $featureDiffs += [PSCustomObject]@{ Text="[CHANGED] $code - $oDesc -> $nDesc"; Color=[System.Drawing.Color]::Yellow }
                    }
                }

                $oldJuris = @($oldData.PackageJuris[$pkg])
                $newJuris = @($newData.PackageJuris[$pkg])
                $addedJuris = $newJuris | Where-Object { $_ -notin $oldJuris }
                $removedJuris = $oldJuris | Where-Object { $_ -notin $newJuris }

                $oldUser = if ($oldData.PackageUsers.ContainsKey($pkg)) { $oldData.PackageUsers[$pkg] } else { $null }
                $newUser = if ($newData.PackageUsers.ContainsKey($pkg)) { $newData.PackageUsers[$pkg] } else { $null }
                $userChanged = (($null -ne $oldUser -or $null -ne $newUser) -and ($oldUser -ne $newUser))

                $oldId = if ($oldData.PackageIds.ContainsKey($pkg)) { $oldData.PackageIds[$pkg] } else { $null }
                $newId = if ($newData.PackageIds.ContainsKey($pkg)) { $newData.PackageIds[$pkg] } else { $null }
                $idChanged = (($null -ne $oldId -or $null -ne $newId) -and ($oldId -ne $newId))

                if ($featureDiffs.Count -gt 0 -or $addedJuris.Count -gt 0 -or $removedJuris.Count -gt 0 -or $userChanged -or $idChanged) {
                    $hasDiff = $true
                    Append-Text "=== PACKAGE MODIFIED: $pkg ===" [System.Drawing.Color]::Yellow $true
                    
                    if ($idChanged) {
                        $oIdDisp = if ($oldId) { $oldId } else { "(none)" }
                        $nIdDisp = if ($newId) { $newId } else { "(none)" }
                        Append-Text "  [CHANGED] Packages ID: $oIdDisp -> $nIdDisp" [System.Drawing.Color]::Yellow
                    }
                    if ($userChanged) {
                        $oUserDisp = if ($oldUser) { $oldUser } else { "(none)" }
                        $nUserDisp = if ($newUser) { $newUser } else { "(none)" }
                        Append-Text "  [CHANGED] No. of User: $oUserDisp -> $nUserDisp" [System.Drawing.Color]::Yellow
                    }
                    foreach ($id in $removedJuris) { Append-Text "  [- JURIS REMOVED] Package Juris ID: $id" [System.Drawing.Color]::FromArgb(255, 76, 76) }
                    foreach ($id in $addedJuris) { Append-Text "  [+ JURIS ADDED] Package Juris ID: $id" [System.Drawing.Color]::Lime }
                    foreach ($fd in $featureDiffs) { Append-Text "  $($fd.Text)" $fd.Color }
        ...
        if (-not $hasDiff) {
            Append-Text "No differences detected! Configurations are identical." [System.Drawing.Color]::Cyan $true
        }
    })
...
Add-SidebarButton $pnlGroupDiag "Check Public IP" 85 $colorBtnDiag { 
    global:Show-ExecutionLogs
    global:Write-Terminal "Resolving Public IP Address..." "Yellow" "ALL"
    try { 
        $ip = (Invoke-RestMethod -Uri 'https://api.ipify.org' -UseBasicParsing -ErrorAction Stop)
        global:Write-Terminal "Public IP Resolved: $ip" "Lime" "ALL" 
    } catch { 
        global:Write-Terminal "Failed to resolve Public IP: $($_.Exception.Message)" "Red" "ALL" 
    } 
}
Add-SidebarButton $pnlGroupDiag "Run Diagnostics" 130 $colorBtnDiag { 
    global:Show-ExecutionLogs
    global:Write-Terminal "--- INITIATING SYSTEM AUDIT & CLEANUP ---" "Cyan" "ALL" 
    try {
        $freeSpace = [math]::Round(((Get-WmiObject Win32_LogicalDisk -Filter "DeviceID='C:'").FreeSpace / 1GB), 2)
        global:Write-Terminal "C: Drive Free Space: $freeSpace GB" "LightGray" "ALL"
        
        $mem = Get-WmiObject Win32_OperatingSystem
        $freeMem = [math]::Round(($mem.FreePhysicalMemory / 1024), 2)
        global:Write-Terminal "Free Physical Memory: $freeMem MB" "LightGray" "ALL"
        
        $cpu = Get-WmiObject Win32_Processor | Measure-Object -Property LoadPercentage -Average | Select-Object -ExpandProperty Average
        global:Write-Terminal "Average CPU Load: $cpu%" "LightGray" "ALL"
        
        $topProc = Get-Process | Sort-Object WS -Descending | Select-Object -First 1
        $procMem = [math]::Round($topProc.WS / 1MB, 2)
        global:Write-Terminal "Highest Memory Process: $($topProc.Name) ($procMem MB)" "LightGray" "ALL"

        global:Write-Terminal "Clearing System & User Temp files..." "Yellow" "ALL"
        Remove-Item -Path "$env:TEMP\*" -Recurse -Force -ErrorAction SilentlyContinue
        Remove-Item -Path "C:\Windows\Temp\*" -Recurse -Force -ErrorAction SilentlyContinue

        global:Write-Terminal "Clearing Browser Caches..." "Yellow" "ALL"
        ...
        global:Write-Terminal "System Audit & Cleanup Complete." "Lime" "ALL"
    } catch {
        global:Write-Terminal "System Audit encountered an error: $($_.Exception.Message)" "Red" "ALL"
    }
}
...
if ($null -ne $script:form) {
    $script:form.Add_Shown({ 
        try {
            ...
            $Url_GitHub = "https://github.com/gobikrish90/MyScripts/raw/main/Phoenix%20Installation%20Master_20260925_2132.zip"
            $Url_GDrive = "https://drive.google.com/uc?export=download&id=10RxuJaWwqR1S6lbkjL0-_AXddCwOARYI"
